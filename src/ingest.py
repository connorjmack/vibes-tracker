...
def resolve_channel_id(youtube, handle, cache, logger, quota_tracker):
...
def get_recent_videos(youtube, channel_id, channel_name, limit, logger, quota_tracker):
    ...
    try:
        for cluster_name, handles in clusters_config.items():
            ...
            for handle in handles:
                ...
                cid = resolve_channel_id(youtube, handle, channel_id_cache, logger, quota_tracker)
                if not cid:
                    continue

                # 2. Fetch Videos (Cheap calls: 2 units per channel)
                videos = get_recent_videos(youtube, cid, handle, config.ingest.videos_per_channel, logger, quota_tracker)
